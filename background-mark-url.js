...
const fetchMarkUrls = async () => {
  ...
  //
  const apiUrl = `https://www.genaianywhere.com/api/mark-url?userId=${userId}&languageCode=${languageCode}`;
  ...
  if (!fetchedInitialMarkUrls) { // 초기 마크 주소 가져오기
    ...
    if (changeInfo.status === "complete" && tab.active) {
      ...
      if (!urlPattern.test(tab.url)) {
        ...
        //
        const url = `https://www.genaianywhere.com/api/mark-url?userId=${userId}&languageCode=${languageCode}`;
        //console.log(url);
